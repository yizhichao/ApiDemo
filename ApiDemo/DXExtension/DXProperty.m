...
@interface DXProperty()
@property (strong, nonatomic) NSMutableDictionary *propertyKeysDict;
@property (strong, nonatomic) NSMutableDictionary *objectClassInArrayDict;
@end

@implementation DXProperty
...
{
    DXProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    DXExtensionAssertParamNotNil(property);
    ...
    _type = [DXPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                DXPropertyKey *propertyKey = [[DXPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                DXPropertyKey *subPropertyKey = [[DXPropertyKey alloc] init];
                subPropertyKey.type = DXPropertyKeyTypeArray;
